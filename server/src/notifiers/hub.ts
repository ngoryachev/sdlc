...
export function startNotifierHub(app: App, notifiers: Notifier[], publicUrl: string): () => void {
  ...
  const off = events.on((e) => {
    switch (e.type) {
      ...
      case 'task.status': { const p = e.payload as { task: Task; from: string; to: string }; if (!['succeeded', 'merged', 'closed', 'failed', 'aborted', 'pr_open', 'paused'].includes(p.to)) return; for (const n of notifiers) if (n.onTaskStatus) void safe(n, () => n.onTaskStatus!(p.task, p.from, p.to, link(`/tasks/${p.task.id}`))); break; }

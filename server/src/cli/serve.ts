...
export function registerServe(program: Command) {
  program.command('serve').description('Start the orchestrator: engine + API + UI')
    ...
    .action(async (o) => {
      ...
      if (app.config.telegram.enabled && app.config.telegram.bot_token) notifiers.push(new TelegramNotifier(app, { botToken: app.config.telegram.bot_token, chatId: app.config.telegram.chat_id, token }));
      ...
      const server = serve({ fetch: hono.fetch, hostname: app.config.server.host, port: app.config.server.port }, (info) => {
        ...
        console.log(`open: ${publicUrl}/?t=${token}`);
      });
